...
class BlueprintControllerTest {
    ...
    void checklistFormShowsFirstVisitorGuideAndSampleRoute() throws Exception {
        mockMvc.perform(get("/blueprint/form"))
                ...
                .andExpect(content().string(containsString("check-progress-stepper")))
                .andExpect(content().string(containsString("sample-patterns")))
                .andExpect(content().string(containsString("サンプル選択")))
                .andExpect(content().string(containsString("設計条件を整理")))
                .andExpect(content().string(containsString("承認・監査を確認")))
                .andExpect(content().string(containsString("設計候補生成")))
                .andExpect(content().string(containsString("check-status-card")))
                .andExpect(content().string(containsString("構造化された設計条件を直接編集できます")))
    ...
    void getBlueprintResultDisplaysStoredResult() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/blueprint/result")
                ...
                .andExpect(content().string(containsString("<div class=\"step4-artifact-head step4-artifact-head-stacked\"><span class=\"step4-tag step4-tag-top\">Markdown</span><h3 style=\"white-space: nowrap;\">API&amp;MCP設計書</h3></div>")))
                .andExpect(content().string(containsString("<div class=\"step4-artifact-head step4-artifact-head-stacked\"><span class=\"step4-tag step4-tag-top\">Implementation</span><h3 style=\"white-space: nowrap;\">AI実装指示書</h3></div>")))
