...
class BlueprintControllerTest {
    ...
    void getRootShowsFirstVisitorGuideAndSampleRoute() throws Exception {
        mockMvc.perform(get("/"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("業務要件から、Spring REST API設計とMCP tools/resources/prompts設計候補を同時生成")))
                .andExpect(content().string(containsString("API設計書、MCP tools/resources/prompts、API/MCP対応表、AI実装指示書")))
                .andExpect(content().string(containsString("Codex等の実装支援AIへの入力")))
